...
We're getting plausible predictions for most building means but some are estimated 
better than others and some have very large uncertaintys. This indicates that there
...
comp_model_NB_hier <- stan_model('stan_programs/hier_NB_regression2.stan')
```
...
\text{Var}(\texttt{mo}_1) = \text{Var}(\rho \texttt{mo}_{0}) + \text{Var}(\epsilon_1) \quad (\textrm{by independence of } \epsilon_t \, \text{and} \, \epsilon_{t-1})\\
...
\mathbb{E}(\texttt{mo}_1) = \mathbb{E}(\rho \, \texttt{mo}_{0}) + 0 \quad (\mathbb{E}(\epsilon_t) = 0 \, \textrm{by assumption})\\
\mathbb{E}(\texttt{mo}_1) = \rho \, \mathbb{E}(\texttt{mo}_{1}) \\
\mathbb{E}(\texttt{mo}_1)(1 - \rho) = 0  \\
\mathbb{E}(\texttt{mo}_1) = 0/(1 - \rho)  
$$

which for $\rho \neq 1$ yields $\mathbb{E}(\texttt{mo}_{1}) = 0$. So, 
