...
```{r runNBoverfake}
fitted_model_NB <- sampling(comp_model_NB, data = stan_dat_fake_NB, chains = 4, cores = 4)
posterior_alpha_beta_NB <- as.matrix(fitted_model_NB, pars = c('alpha','beta','beta_super','beta_sq_foot','inv_prec'))
```
...
```{r}
stan_dat_hier$mo_idx <- as.integer(as.factor(pest_data$date))
stan_dat_hier$M <- max(stan_dat_hier$mo_idx)
```
...
stan_dat_hier$meta_data <- stan_dat_hier$building_data
...
stan_dat_hier$sq_foot_pred <- log(unique(building_data[,'total_sq_foot'])/1e4)
...
tot_profit <- sweep(samps_rev$pp_rev,c(3),STATS = costs, FUN = '-')
mean_rev <- apply(tot_profit,c(2,3),median)
lower_rev <- apply(tot_profit,c(2,3),quantile,0.25)
upper_rev <- apply(tot_profit,c(2,3),quantile,0.75)
mean_complaints <- apply(samps_rev$pp_y,c(2,3),mean)
lower_complaints <- apply(samps_rev$pp_y,c(2,3),quantile,0.25)
upper_complaints <- apply(samps_rev$pp_y,c(2,3),quantile,0.75)
...
ggplot(data = rev_df, aes(x = traps, y = rev)) +
