...
```{r prep-data}
t <- max(table(pest_data$building_id))
J <- length(unique(pest_data$building_id))
pest_data <- pest_data %>%
  mutate(building_fac = as.factor(building_id),
         building_idx = as.integer(building_fac),
         ids = rep(1:t,J),
         mo_idx = lubridate::month(date))
meta_data <- unique(pest_data[,c('building_idx','live_in_super', 'age_of_building', 'total_sq_foot',
...
```{r stan-data}
stan_dat_simple <- with(pest_data, list(complaints = complaints,
                                         traps = traps,
                                         N = length(traps)))
```
...
fake_gen_data$traps <- rnorm(fake_gen_data$N)
```
...
stan_dat_hier <- with(pest_data, list(complaints = complaints,
                                       ...
                                       meta_data = scale(meta_data[,c('live_in_super',
                                                                'age_of_building',
                                                                'average_tenant_age',
