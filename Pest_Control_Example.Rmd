...
```{r load-data}
pest_data <- readRDS('data/building_data_20180727_time_trend.RDS')
str(pest_data)
```
...
Then do Posterior Predictive Checks. One common check for regression models is
...
```
This is a plot of the kernel densities of the observed data ($y$) and 200 
simulations of $y$ from the posterior predictive distribution ($y_rep$). If the 
model was fitting the data well, there would be little difference between the 
observed density and the simulated density. However, as you can see here, the 
simulated density is not as dispersed as the kernel density of the observed
data, and doesn't seem to capture the rate of zeros in the observed data. This
indicates that the Poisson model may not be the best model to use for this data.
...
Ideally these would be centered around zero on the y-axis with the bulk of the
residuals between -1 and 1 (1 standard deviation on the unit scale).  As you can
see here, it looks as though we have more positive residuals than negative,
...
Now we need a new Stan model that uses multiple predictors. Firs we have to
...
We can see that is indeed the case with several large observations that are outside
...
The building-level mean for 26, 62, 70 and to a lesser extent 45 and 13 aren't
modeled well. This indicates that there might be some unobserved building-level
variation that we're not capturing with the current model.
...
$\texttt{mu}$ defined as a length `r N_buildings` vector. 
...
$\texttt{building_data}$, which will have `r N_buildings` rows and four columns:
* live_in_super
* age_of_building
* average_tentant_age
...
pest_data <- pest_data %>%
  mutate(
    building_fac = as.factor(building_id),
...
building_data <- pest_data %>%
    ...
    scale(scale=FALSE) %>%   # subtract means but don't divide by SD 
    as.matrix()
...
This gives $\texttt{mu}$ a $\text{Normal}(\alpha + \texttt{building_data}\, \zeta, \sigma_\mu)$ distribution, but it
decouples the dependence of the density of each element of $\texttt{mu}$ from 
$\texttt{sigma_mu}$ ($\sigma_\mu$). hier_NB_regression_ncp.stan uses the non-centered
parameterization for $\texttt{mu}$. We will examine the effective sample size of the
...
posterior distribution for $\texttt{sigma_kappa}$. 
...
There's definitely some pattern that we're not capturing. See May, August,
October, and November for examples. 
Therefore we might augment our model with a log-additive monthly effect, $\texttt{mo}_t$. 
...
[Ask Jonah how to plot the prior vs. the posterior in bayesplot]

The posterior is pretty much the prior distribution for rho, which is to be expected
because we have so few months of data.
...
[Double check the indices for the log_sq_foot_pred]
```{r run-NB-hier-rev}
stan_dat_hier$log_sq_foot_pred <- pest_data %>% arrange(building_idx) %>% mutate(
  log_exposure = log(total_sq_foot/1e4)
) %>% group_by(building_idx) %>% summarise( log_exposure = first(log_exposure)) %>% .$log_exposure
stan_dat_hier$M_forward <- 12
rev_model <- sampling(comp_rev, data = stan_dat_hier, chains = 4, cores = 4, iter = 2000)
```

Below we've generated our revenue curves for the buildings. These charts will give us 
...
plus monthly replenishment of the bait material is about \$20. We'll also need 
labor for maintaining the bait stations. If there are fewer than 5 traps, our 
in-house maintenance staff can manage the stations (about 1 hour per month of 
work at \$20/hour), but above 5 we need to hire outside pest control to help
out. They're a bit more expensive, so we've put their cost at \$30 / hour. Each
5 traps should require an extra person-hour of work, so that's factored in as
well. The marginal person-person hours above five traps are at the higher
pest-control-labor rate.
...
N_traps <- 20
costs <- 10*(1:N_traps) + (1:N_traps < 5)*(6*20) + (1:N_traps >= 5 & 1:N_traps < 10)*(6*50) + (1:N_traps >= 10 & 1:N_traps < 15)*(6*80) +  + (1:N_traps >= 15) * (6*110)
