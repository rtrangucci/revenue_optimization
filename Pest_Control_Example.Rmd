...
bayesplot::ppc_stat_grouped(stan_dat_simple$complaints, 
                            y_rep[1:200,], group = pest_data$building_idx, stat = 'mean')
```
...
```{r ppc-full-hier}
y_rep <- samps_NB_hier_ncp$y_rep
bayesplot::ppc_dens_overlay(stan_dat_hier$complaints, 
                          y_rep[1:200,])
```
...
bayesplot::ppc_stat_grouped(stan_dat_hier$complaints, 
                            y_rep[1:200,], group = stan_dat_hier$building_idx, stat = 'mean')
```
...
```{r}
mean_y_rep <- colMeans(y_rep)
mean_inv_prec <- mean(samps_NB_hier_ncp$inv_prec)
...
samps_NB_hier_slopes <- 
  rstan::extract(
    fitted_model_NB_hier_slopes,
    pars = c('y_rep','inv_prec','sigma_beta')
  )
```
...
y_rep <- samps_NB_hier_slopes$y_rep
bayesplot::ppc_dens_overlay(stan_dat_hier$complaints, 
                            y_rep[1:200,])
```
...
bayesplot::ppc_stat_grouped(stan_dat_hier$complaints, 
                            samps_NB_hier_slopes$y_rep[1:200,], group = stan_dat_hier$mo_idx, stat = 'mean')
```
...
[This doesn't quite come out from any residuals because we have so few weeks, we could 
...
```{r}
samps_NB_hier_mos <- rstan::extract(fitted_model_NB_hier_mos)
```
In the interest of brevity, we won't go on expanding the model. 

```{r ppc-full-hier-mos}
bayesplot::ppc_dens_overlay(stan_dat_hier$complaints, 
                            samps_NB_hier_mos$y_rep[1:200,])
```

```{r}
bayesplot::ppc_stat_grouped(stan_dat_hier$complaints, 
                            samps_NB_hier_mos$y_rep[1:200,], group = stan_dat_hier$mo_idx, stat = 'mean')
```
